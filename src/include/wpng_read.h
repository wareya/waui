...
static uint16_t apply_gamma_u16(uint16_t val, double gamma)
{
    ...
    // this is intentional behavior on libpng's part.... for some reason....
...
enum {
    ...
    WPNG_READ_ERROR_ON_BAD_ANCILLARY_CRC = 16, // treat chunks with bad CRCs like unknown chunks instead of ignoring them
...
typedef struct {
    ...
    size_t bytes_per_scanline;    // scanline byte count
    uint32_t width;               // width
    uint32_t height;              // height
    float gamma;                  // gamma (-1 if unset or srgb)
    uint8_t bytes_per_pixel;      // bytes per pixel
    uint8_t is_16bit;             // is 16 bit or not (decoder output)
    uint8_t was_16bit;            // was originally 16 bit or not (original png file)
    uint8_t was_srgb;             // png file specified that it was srgb or not
    uint8_t error;                
} wpng_load_output;
...
typedef struct {
    // for the following three chunks: the CRC has already been checked, and if compressed, the data has already been decompressed.
    // you do not gain ownership of the pointed text data (neither keyword nor text); if you want to hold onto it, you must copy it.
    // byte_count is the number of text bytes, not the original length of the chunk.
    
    // the string `text` points to is null terminated. however, if the original PNG file doesn't
    // comply with the PNG spec, it might contain null bytes before the null terminator.
    
    // the text in "text" and "ztxt" chunks is supposed to be Latin-1, but this is not checked.
    // the text in "itxt" chunks is UTF-8.
    
    void (*found_text)(uint8_t * keyword, uint8_t * text, size_t byte_count, uint8_t * userdata);
    void (*found_ztxt)(uint8_t * keyword, uint8_t * text, size_t byte_count, uint8_t * userdata);
    // "keyword" points to four bytes of Latin-1 text; it is not null-terminated
    // "lang_tag" points to a null-terminated string of Latin-1 text
    // "trns_keyword" points to a null-terminated string of Latin-1 text
    void (*found_itxt)(uint8_t * keyword, uint8_t * lang_tag, uint8_t * trns_keyword, uint8_t * text, size_t byte_count, uint8_t * userdata);
    
    // chunk_data is a pointer to the raw data, not including the length or header.
    // byte_count is the chunk's length field, i.e. the number of bytes between the header and CRC
    // the chunk's CRC has already been checked. (this can be overridden by a flag passed to wpng_load)
    
    // this callback is called after the text chunk callbacks are called.
    void (*known_chunk)(uint8_t * chunk_name, uint8_t * chunk_data, size_t byte_count, uint8_t * userdata);
    
    // this callback needs to return whether your application knows the given chunk (1 if it does, 0 if it doesn't).
    uint8_t (*unknown_chunk)(uint8_t * chunk_name, uint8_t * chunk_data, size_t byte_count, uint8_t * userdata);
...
// `callbacks` is allowed to be null, or point to an object with valid or null pointers
static void wpng_load(byte_buffer * buf, uint32_t flags, wpng_load_output * output)//, wpng_load_callbacks * callbacks)
{
    assert(output);
    assert(buf);
    assert(buf->data);
    ...
    while (buf->cur < buf->len)
    {
        ...
        /*
        else if (memcmp(name, "tEXT", 4) == 0)
        {
            if (callbacks && callbacks->found_text)
            {
                callbacks->found_text
            }
        }
        */
