...
struct Color
{
    ...
    {
        return Color {
            uint8_t((uint16_t)r*other.r/255),
            uint8_t((uint16_t)g*other.g/255),
            uint8_t((uint16_t)b*other.b/255),
            uint8_t((uint16_t)a*other.a/255)
        };
    }
};
...
struct WaEvent
{
    enum Type
    {
        ...
        FOCUS = 0x10000, // Synthesized by the WaUI object. Do not construct yourself.
        DEFOCUS, // Synthesized by the WaUI object. Do not construct yourself.
    };
    enum Action
    {
        INVALID = 0,
...
struct WaControl
{
    ...
    Color modulate_dynamic = Color::WHITE;
    ...
    Color compute_full_modulate()
    {
        return modulate_dynamic * modulate;
    }
};
...
struct WaUI
{
    ...
    uint64_t load_texture(WaRenderAPI * api, const std::string & name, const uint8_t * data, size_t size, WaImageData * img_out);
    ...
    //std::unordered_map<std::string, WaImageData> loaded_images;
    std::unordered_map<std::string, uint64_t> loaded_textures;
    std::unordered_map<std::string, Vec2> loaded_texture_sizes;
...
struct WaButton
{
    ...
    static bool handle_event(WaControl * control, WaUI * ui, WaEvent event, Vec2 pos_offset)
    {
        ...
        {
            control->modulate_dynamic = Color{255, 255, 255, 255};
            return false;
        }
        ...
        {
            control->modulate_dynamic = Color{200, 220, 255, 255};
...
struct WaScroller
{
    static void init(WaControl * control)
    {
        //auto data = control->type_info.set_data(new WaScrollerData());
        //data;
    }
    static void destruct(WaControl * control)
    {
...
struct WaLineEdit
{
    ...
    static void render(WaControl * control, WaUI * ui, WaRenderAPI * api)
    {
        ui->render_ninepatch(api, ui->micro_bg_texture, {{0, 0}, control->rect.size}, {{1, 1}, {4, 4}}, {{2.5, 2.5}, {1, 1}}, ui->loaded_texture_sizes["micro_bg_texture"], Color::OFFWHITE);
        ...
        if (control->focused)
            ui->render_rect(api, {{cursor_x - 1, pad.y}, {1, ui->string_get_height("|")}}, control->compute_full_modulate());
...
uint64_t WaUI::load_texture(WaRenderAPI * api, const std::string & name, const uint8_t * data, size_t data_size, WaImageData * img_out)
{
    ...
    if (img_data.bytes_per_pixel == 1 or img_data.bytes_per_pixel == 2)
    {
        uint8_t new_count = 2 + img_data.bytes_per_pixel;
        uint8_t * new_data = (uint8_t *)malloc(img_data.width * img_data.height * new_count);
        size_t j = 0;
        size_t i = 0;
        if (img_data.bytes_per_pixel == 1)
        {
            for (size_t y = 0; y < img_data.height; y += 1)
            {
                for (size_t x = 0; x < img_data.width; x += 1)
                {
                    new_data[j++] = img_data.data[i];
                    new_data[j++] = img_data.data[i];
                    new_data[j++] = img_data.data[i];
                    i++;
                }
        ...
        free(img_data.data);
        img_data.data = new_data;
        img_data.bytes_per_pixel = new_count;
    }
    
    test_texture = api->texture_create(userdata, img_data.width, img_data.height, true, img_data.bytes_per_pixel, img_data.data);
    
    if (!img_out)
        free(img_data.data);
    else
        *img_out = {img_data.data, img_data.width, img_data.height, img_data.bytes_per_pixel};
    
    loaded_textures.insert({name, test_texture});
    loaded_texture_sizes.insert({name, {(float)img_data.width, (float)img_data.height}});
        ...
        for (uint32_t x0 = 0; x0 < font_cols*font_char_w; x0 += font_char_w)
        {
            ...
            for (uint32_t y = y0; y < y0 + font_char_h; y += 1)
            {
                for (uint32_t x = x0; x < x0 + font_char_w; x += 1)
                {
                    ...
                    auto c = font_img_data.data[index];
                    if (c > 127)
                    {
